...
# Disaster types that, absent cloud constraints, prefer optical imagery.
_OPTICAL_DISASTERS = {"earthquake", "landslide"}
# Disaster types that should always use SAR.
_SAR_DISASTERS = {"flood"}


def select_satellite(
    disaster_type: str, cloud_cover: Optional[float] = None
) -> str:
    """Pick the Sentinel mission for a disaster type.

    - flood -> Sentinel-1 (SAR; sees through cloud and works day/night).
    - earthquake / landslide -> Sentinel-2 (optical).
    - If `cloud_cover` is provided and exceeds CLOUD_COVER_THRESHOLD, an
      otherwise-optical choice is switched to Sentinel-1.

    Unknown disaster types default to optical (Sentinel-2), still subject to the
    cloud-cover fallback. Returns "sentinel-1" or "sentinel-2".
    """
    ...
    if disaster in _SAR_DISASTERS:
        ...
        logger.warning(
            "Unknown disaster type %r; defaulting to optical (Sentinel-2)",
            disaster_type,
        )
        satellite = SENTINEL_2
    ...
    if (
        ...
        logger.info(
            "Cloud cover %.1f%% exceeds %.0f%% threshold; "
            "switching from Sentinel-2 to Sentinel-1 (SAR)",
            cloud_cover,
            CLOUD_COVER_THRESHOLD,
        )
        satellite = SENTINEL_1

    logger.info("Selected %s for disaster type %r", satellite, disaster_type)
    return satellite
...
if __name__ == "__main__":
    ...
    # Mission selection (offline; no network needed).

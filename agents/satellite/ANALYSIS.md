...
- **(b) — STILL OPEN, confirmed by direct read of
  `agents/hazard/analyzer.py:173-224` this session.** `analyze_flood`'s
  LLM-facing prompt (lines 180-185) correctly branches `index_label`/
  `index_context` on `satellite_type`. Its **deterministic fallback** (lines
  211-220, only reached if the LLM call fails) applies flat
  `flood_index > 0.5`/`> 0.3` NDWI-scale thresholds to `mean_value`
  regardless of whether it's an NDWI ratio or a SAR dB number. Since SAR dB
  values are rarely in [0, 1] and structurally almost never exceed 0.5, this
  fallback path biases toward **under-stating** flood risk on a genuine S1
  flood signal (false negative, not false alarm) whenever it fires. Not hit
  on any run where the LLM call succeeds; latent otherwise. This is outside
  this agent's own code (it's `agents/hazard/analyzer.py`) but is flagged
  here because it's the direct downstream consequence of a contract gap on
  this agent's side: `_normalise_satellite_payload` (`agents/hazard/agent.py:81-99`)
  never carries `index_calibrated`/`index_units` into hazard's `analysis`
  dict at all — only `index_type`, `confidence`, `needs_verification` are
  copied (confirmed by direct read). So even if hazard's fallback were fixed
  to branch on `satellite_type`, it still couldn't branch on the more
  precise calibration fields without a second, coupled fix.
...
|---|---|---|---|---|---|
| 1 | **Hazard's deterministic flood fallback still applies NDWI-scale thresholds to a SAR-dB `mean_value`, and `index_calibrated`/`index_units` never reach hazard's `analysis` dict at all** — so even fixing the fallback to branch on `satellite_type` wouldn't let it branch on calibration status without a second, coupled fix in `_normalise_satellite_payload`. | contract | High but conditional (only fires when hazard's LLM call fails); biases toward under-stating real S1 flood signal (false negative). | Low-medium (branch the fallback on `satellite_type`; add `index_calibrated`/`index_units` to `_normalise_satellite_payload`'s `analysis{}`) | agents/hazard/analyzer.py:211-220; agents/hazard/agent.py:81-99 |
| 2 | **SAR backscatter is uncalibrated raw-DN log with no calibration LUT/speckle filter/terrain correction; class thresholds (-13/-15/-18 dB) have no physical basis.** Honestly labeled, not fixed — every S1 flood run (precisely the case chosen because optical is unusable) ships a classification that isn't scientifically defensible. | science | High | High (real remote-sensing calibration engineering) | processor.py SAR index/class code |
| 3 | **Confidence's new legibility fields (`confidence_basis`, `evidence_count`) don't cross the satellite→hazard boundary** — the tracker-level fix (§5.3) is real, but `_normalise_satellite_payload` still only copies `confidence` itself, not the basis that would let a downstream consumer tell "no evidence gathered" apart from "evidence contradicts." | contract | Medium-high (the exact ambiguity the fields were built to resolve still exists one hop downstream) | Low (two more keys in `_normalise_satellite_payload`'s `analysis{}`) | agents/hazard/agent.py:81-99; confidence_tracker.py |
| 4 | **NDWI/NDVI thresholds (0.3/0.5 water, 0.2 damage) were tuned against L1C and not revalidated since the L2A switch.** | science | Medium-high, affects every S2-path run | Medium (dedicated science-validation pass against known ground truth) | processor.py comments; CLAUDE.md |
| 5 | **NDVI earthquake/landslide damage detection is single-scene absolute-threshold, not bi-temporal**, conflating damage with naturally bare terrain. | science | Medium, every S2 earthquake/landslide run | High (requires a pre-event baseline fetch + differencing feature) | processor.py NDVI code |
| 6 | **Sentinel-1 has no per-band Nodes download path** — every S1 candidate is a full 1.2-1.7 GB archive fetch, unlike S2's per-band resilience. | performance | Medium, hits the most urgent (cloud-obscured) cases hardest | Medium (extend Nodes-tree mapping to S1 GRD `measurement/*.tiff` layout) | processor.py |
| 7 | **Confidence tracker is still a hand-picked heuristic, not a calibrated estimate** — weights/penalties aren't fit to accuracy data; the SAR-path evidence/concern asymmetry (§5.1) means S1 and S2 runs aren't comparable on the same confidence scale even after the legibility fix. | science/contract | Medium (the legibility fix reduces but doesn't eliminate the risk of over-trusting the number) | High (needs historical accuracy data this repo doesn't have) | confidence_tracker.py |
| 8 | **`artifacts_incomplete`/`failed_artifacts` exist but nothing yet forces a downstream consumer to check them** — a `None` artifact URL can still slip through to the frontend/report if the new flags are ignored, same structural risk as before the fix, just with a flag now available. | contract | Low-medium (visible if checked, silent if not) | Low (documentation/consumer-side check, not a satellite-agent change) | agent.py |
| 9 | **Per-city rendering is now flag-gated but still off by default with no memory-sizing work done** — `ENABLE_PER_CITY_ARTIFACTS=true` would multiply peak RSS on top of an already ~9.6 GB single-mosaic peak observed live. | performance (design) | Low (correct default, just a known limitation) | Medium (memory sizing/headroom work, tracked as a prerequisite) | CLAUDE.md 2026-07-27 log |
| 10 | **Coverage tier day windows (0/±3/±7/±14) leave S1 tiers 2/3 as structural near-no-ops** given the measured ~11-day actual revisit — not incorrect, but adds negligible search latency for effectively zero benefit on the S1 path. | correctness (design) | Low | Low (design review only, already done — left unchanged intentionally) | CLAUDE.md's tier-window revisit analysis |
| — | RESOLVED, not a current gap: DB-persist-swallowed-as-warning, area-unit mislabel, orphaned mosaic set-cover code, permanently-unreachable per-city rendering, `total_zones`/`scene_id` always-NULL, silent R2 partial-upload with no flag. All six confirmed closed in §11 above. | — | — | — | — |

...
Quantifies population and infrastructure impact within hazard zones.

## Responsibilities
- Intersect risk polygons with population/infra datasets
...
- **Credentials:** `agent_config.yaml` (gitignored) holds `agent_id`/`api_key`
  under the `impact_agent` key. Runtime values are loaded from `.env`.
...
- [ ] Implement impact assessment pipeline
